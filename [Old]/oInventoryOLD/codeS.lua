...
function loadInventoryElement(element)
	...
	dbQuery(function(qh, element)
		...
		if rows > 0 then
			for k, row in pairs(res) do
				itemTable[element][tostring(row["type"])][tonumber(row["slot"])] = {
	...
	end, {element}, conn, "SELECT * FROM `items` WHERE `owner` = ?",elementOwnerID)
end
...
function saveOneElementInventory(element)
	if getElementType(element) == "player" then
		...
		for i = 1, row * column do
			if (itemTable[element]["bag"][i]) then
				local dbid = tonumber(itemTable[element]["bag"][i]["dbid"])
				local item = tonumber(itemTable[element]["bag"][i]["id"])
				local value = tostring(itemTable[element]["bag"][i]["value"])
				local count = tonumber(itemTable[element]["bag"][i]["count"])
				local duty = tonumber(itemTable[element]["bag"][i]["duty"])
				local state = tonumber(itemTable[element]["bag"][i]["health"])
				local name = tostring(itemTable[element]["bag"][i]["name"])
				local weaponserial = tostring(itemTable[element]["bag"][i]["weaponserial"])
				dbExec(conn,"UPDATE items SET slot = ?, value = ?, count = ?, type = ?, owner = ?, dutyitem = ?, itemState = ?, name = ?, weaponserial = ? WHERE id = ?",i,value,count,"bag",userid,duty,state,name,weaponserial,dbid)
			end
		end
		for i = 1, row * column do
			if (itemTable[element]["key"][i]) then
				local dbid = tonumber(itemTable[element]["key"][i]["dbid"])
				local item = tonumber(itemTable[element]["key"][i]["id"])
				local value = tostring(itemTable[element]["key"][i]["value"])
				local count = tonumber(itemTable[element]["key"][i]["count"])
				local duty = tonumber(itemTable[element]["key"][i]["duty"])
				local state = tonumber(itemTable[element]["key"][i]["health"])
				local name = tostring(itemTable[element]["key"][i]["name"])
				local weaponserial = tostring(itemTable[element]["key"][i]["weaponserial"])
				dbExec(conn,"UPDATE items SET slot = ?, value = ?, count = ?, type = ?, owner = ?, dutyitem = ?, itemState = ?, name = ?, weaponserial = ? WHERE id = ?",i,value,count,"key",userid,duty,state,name,weaponserial,dbid)
			end
		end
		for i = 1, row * column do
			if (itemTable[element]["licens"][i]) then
				local dbid = tonumber(itemTable[element]["licens"][i]["dbid"])
				local item = tonumber(itemTable[element]["licens"][i]["id"])
				local value = tostring(itemTable[element]["licens"][i]["value"])
				local count = tonumber(itemTable[element]["licens"][i]["count"])
				local duty = tonumber(itemTable[element]["licens"][i]["duty"])
				local state = tonumber(itemTable[element]["licens"][i]["health"])
				local name = tostring(itemTable[element]["licens"][i]["name"])
				local weaponserial = tostring(itemTable[element]["licens"][i]["weaponserial"])
				dbExec(conn,"UPDATE items SET slot = ?, value = ?, count = ?, type = ?, owner = ?, dutyitem = ?, itemState = ?, name = ?, weaponserial = ? WHERE id = ?",i,value,count,"licens",userid,duty,state,name,weaponserial,dbid)
			end
		end
	else
        local ownerid = getElementData(element,getTypeOfElement(element)[2])
		for i = 1, row * column do
			if (itemTable[element][getTypeOfElement(element)[1]][i]) then
				local dbid = tonumber(itemTable[element][getTypeOfElement(element)[1]][i]["dbid"])
				local item = tonumber(itemTable[element][getTypeOfElement(element)[1]][i]["id"])
				local value = tostring(itemTable[element][getTypeOfElement(element)[1]][i]["value"])
				local count = tonumber(itemTable[element][getTypeOfElement(element)[1]][i]["count"])
				local duty = tonumber(itemTable[element][getTypeOfElement(element)[1]][i]["duty"])
				local state = tonumber(itemTable[element][getTypeOfElement(element)[1]][i]["health"])
				local name = tostring(itemTable[element][getTypeOfElement(element)[1]][i]["name"])
				local weaponserial = tostring(itemTable[element][getTypeOfElement(element)[1]][i]["weaponserial"])
				dbExec(conn,"UPDATE items SET slot = ?, value = ?, count = ?, type = ?, owner = ?, dutyitem = ?, itemState = ?, name = ?, weaponserial = ? WHERE id = ?",i,value,count,getTypeOfElement(element)[1],ownerid,duty,state,name,weaponserial,dbid)
			end
		end
	end
end
...
function getElementItems(playerSource, element, playerValue, itemsNow)
	if (element) then
		...
		if (tonumber(playerValue) == 1 or tonumber(playerValue) == 2) then
			if (itemTable == itemsNow) then 
				print("nincs változás")
			else
				print("update")
				triggerClientEvent(playerSource, "setElementItems", playerSource, itemTable, playerValue, element)	
			end
		else
			return itemTable
		end		
...
function takeStatus(playerSource, elementSource, itemData, newStatus)
	if (newStatus) then
		itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemData["id"]))[1]][tonumber(itemData["slot"])]["health"] = newStatus
	end
end
...
function updateItemSlot(playerSource, elementSource, newSlot, oldItem,oldSlot,click)
	if (newSlot and oldItem) then
		if click == 1 then
			itemTable[elementSource][getTypeOfElement(elementSource,tonumber(oldItem["id"]))[1]][tonumber(oldSlot)] = nil
			itemTable[elementSource][getTypeOfElement(elementSource,tonumber(oldItem["id"]))[1]][tonumber(newSlot)] = {
...
function updateItemCount(playerSource, elementSource, itemSlot, newCount,itemData)
	if (newCount) then
		itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemData["id"]))[1]][tonumber(itemSlot)]["count"] = newCount
	end
end
...
function updateItemName(playerSource, elementSource, itemSlot, newName, itemData)
	if (newName) then
		itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemData["id"]))[1]][tonumber(itemData["slot"])]["name"] = newName
	end
end
...
function updateItemValue(playerSource, elementSource, itemSlot, newValue, itemData)
	if (newValue) then
		itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemData["id"]))[1]][tonumber(itemData["slot"])]["value"] = newValue
	end
end
...
function deleteItem(playerSource, elementSource,itemData,state)
	...
	if (elementSource) then
		...
		dbExec(conn,"DELETE FROM `items` WHERE `id` = ?",itemData["dbid"])
		itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemData["id"]))[1]][tonumber(itemData["slot"])] = nil
		getElementItems(playerSource, elementSource, 2)
	end
end
	...
	dbQuery(function(q, element)
		...
		if rws > 0 then
			...
			for k,r in pairs(rs) do
				dbExec(conn, "DELETE FROM `actionbaritems` WHERE `itemdbid`=?",r["itemdbid"])
				triggerClientEvent(element,"delActionBarSlot",element,element,r["actionslot"])
			end
		end
	end, {element}, conn,"SELECT * FROM `actionbaritems` WHERE `itemdbid` = ?", itemDBID)
end
...
function createNewItem(playerSource, elementSource, itemID, itemSlot, itemCount, itemDuty, itemValue,itemHealth,itemName,weaponserial,oldDBID)
	if (itemID and itemSlot) then
		...
		dbQuery(function(qh,itemID,elementOwnerID,itemSlot)
			...
			if id > 0 then
				itemTable[elementSource][getTypeOfElement(elementSource,tonumber(itemID))[1]][tonumber(itemSlot)] = {
					["dbid"] = tonumber(id),
					["id"] = tonumber(itemID),
		...
		end,{itemID,elementOwnerID,itemSlot},conn, "INSERT INTO `items` SET `itemid` = ?, `slot` = ?, `owner` = ?, `value` = ?, `count` = ?, `type` = ?, `dutyitem` = ?, `itemState` = ?, `name` = ?, `weaponserial` = ?",itemID, itemSlot, elementOwnerID, itemValue, itemCount, getTypeOfElement(elementSource,tonumber(itemID))[1], itemDuty,itemHealth,itemName,weaponserial)
	end
end
...
function tradeItem(playerSource, elementSource, activeElementSource, activeItem,click)
	if (playerSource and elementSource and activeItem) then
		if click == 1 then
			...
			if not itemTable[elementSource] then
				itemTable[elementSource] = {
					["bag"] = {},
					["key"] = {},
					["licens"] = {},
					["vehicle"] = {},
			...
			if (pSlotState and pSlotID) then
				...
				itemTable[activeElementSource][getTypeOfElement(activeElementSource,tonumber(activeItem["id"]))[1]][tonumber(activeItem["slot"])] = nil
				itemTable[elementSource][getTypeOfElement(elementSource,tonumber(activeItem["id"]))[1]][tonumber(pSlotID)] = {
...
function getElementItemsWeight(element)
	...
	if elementType == "player" then
		for i = 1, row * column do
			if (itemTable[element]["bag"][i]) then
				bagWeight = bagWeight + (getItemWeight(itemTable[element]["bag"][i]["id"]) * itemTable[element]["bag"][i]["count"])
			end
		end
	elseif elementType == "vehicle" then 
		for i = 1, row * column do
			if (itemTable[element]["vehicle"][i]) then
				vehWeight = vehWeight + (getItemWeight(itemTable[element]["vehicle"][i]["id"]) * itemTable[element]["vehicle"][i]["count"])
			end	
		end
	elseif elementType == "object" then 
		for i = 1, row * column do
			if (itemTable[element]["object"][i]) then
				objectWeight = objectWeight + (getItemWeight(itemTable[element]["object"][i]["id"]) * itemTable[element]["object"][i]["count"])
			end	
		end
	end
...
function hasItem(element,item,value)
	...
	for i = 1, row * column do
		if (itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]) then
			if tonumber(itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["id"]) == tonumber(item) then
				if value == -1 then
					if tonumber(itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["value"]) < 4 then
						return true,itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["id"],itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["value"]
					end
				elseif value == "?" then 
					return true,itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["id"],itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["value"],itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]
				else
					if tonumber(itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["value"]) == tonumber(value) then
						return true,itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["id"],itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]["value"],itemTable[element][getTypeOfElement(element,tonumber(item))[1]][i]
					end
				end
			end
	...
	if (playerSource and itemID and itemValue and itemCount and itemDuty) then
		...
		dbQuery(function(qh,itemID,elementOwnerID,pSlotID)
			...
			if id > 0 then
				...
				itemTable[playerSource][getTypeOfElement(playerSource,tonumber(itemID))[1]][tonumber(pSlotID)] = {
					["dbid"] = tonumber(id),
					["id"] = tonumber(itemID),
					["value"] = tostring(itemValue),
					["count"] = tonumber(itemCount),
		...
		end,{itemID,elementOwnerID,pSlotID},conn, "INSERT INTO `items` SET `itemid` = ?, `slot` = ?, `owner` = ?, `value` = ?, `count` = ?, `type` = ?, `dutyitem` = ?, `itemState` = ?, `name` = ?, `weaponserial` = ?, `amount` = ? ",itemID, pSlotID, elementOwnerID, itemValue, itemCount, getTypeOfElement(playerSource,tonumber(itemID))[1], itemDuty, itemState,itemName,weaponSerial, 0)
	end
end
		...
		if who and category and slot then
			if category == "bag" or category == "key" or category == "licens" then
				...
				if type(slot) == "number" then
					...
					if targetPlayer then
						if(itemTable[targetPlayer][category])then
							if(itemTable[targetPlayer][category][slot])then
								outputChatBox(core:getServerPrefix("green-dark", "Inventory", 3).."Sikeresen töröltél "..color..getPlayerName(targetPlayer):gsub("_", " ").."#ffffff -tól/től egy "..color..getItemName(itemTable[targetPlayer][category][slot]["id"],itemTable[targetPlayer][category][slot]["value"]).."#ffffff -t.",playerSource,61,122,188, true)
								outputChatBox(core:getServerPrefix("red-dark", "Inventory", 1)..getElementData(targetPlayer,"user:adminnick").."#ffffff törölt tőled egy "..color..getItemName(itemTable[targetPlayer][category][slot]["id"],itemTable[targetPlayer][category][slot]["value"]).."#ffffff -t.",targetPlayer,61,122,188, true)
								
								admin:sendMessageToAdmins(playerSource, "törölt egy #db3535"..getItemName(itemTable[targetPlayer][category][slot]["id"],itemTable[targetPlayer][category][slot]["value"]).."#557ec9-t tőle: #db3535"..getPlayerName(targetPlayer):gsub("_", " ").."#557ec9!")
								if(weaponModels[itemTable[targetPlayer][category][slot]["id"]])then
									delAttachWeapon(targetPlayer,itemTable[targetPlayer][category][slot]["id"],itemTable[targetPlayer][category][slot]["value"],itemTable[targetPlayer][category][slot]["dbid"])
								end
								deleteItem(targetPlayer,targetPlayer,itemTable[targetPlayer][category][slot])
...
function addPhone(playerSource,typ)
	...
	end, {playerSource}, conn,"INSERT INTO phones SET phoneNumber = ?, type = ?, date = NOW()",tonumber(thePhoneNumber),tostring(typ))
end
...
function getFreeSlot(elementSource, forItemID)
	local nextSlot = 1
	for i = 1, (row * column) do
		if not (itemTable[elementSource][getTypeOfElement(elementSource,tonumber(forItemID))[1]][i]) then
...
function takeItem(playerSource,item)
	local elem = 0
	if itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]] then
		for i = 1, row * column do
			if (itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]) then
				if itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["id"] == item then
					elem = elem+1
					if elem == 1 then
						if itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["count"] > 1 then
							itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["count"] = itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["count"]-1
							getElementItems(playerSource, playerSource, 2)
						else
							if weaponModels[itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["id"]] then
								delAttachWeapon(playerSource,itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["id"],itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["value"],itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i]["dbid"])
							end
							deleteItem(playerSource,playerSource,itemTable[playerSource][getTypeOfElement(playerSource,tonumber(item))[1]][i])
						end
					end
				end
			end
...
function takeAllItem(item,value)
	for k,v in ipairs(getElementsByType("vehicle")) do
		...
		if vehID > 0 then
			if itemTable[v] then
				if itemTable[v][getTypeOfElement(v,tonumber(item))[1]] then
					for i = 1, row * column do
						if itemTable[v][getTypeOfElement(v,tonumber(item))[1]][i] then
							if itemTable[v][getTypeOfElement(v,tonumber(item))[1]][i]["id"] == tonumber(item) and tonumber(itemTable[v][getTypeOfElement(v,tonumber(item))[1]][i]["value"]) == tonumber(value) then
								itemTable[v][getTypeOfElement(v,tonumber(itemTable[v][getTypeOfElement(v,tonumber(item))[1]][i]["id"]))[1]][tonumber(i)] = nil
	...
	dbQuery(function(query)
		...
		for k, row in pairs (query) do
			dbExec(conn,"DELETE FROM `items` WHERE `id` = ?",row["id"])
		end
	end, conn, "SELECT * FROM `items` WHERE `value` = ? AND `itemid` = ?",tonumber(value),tonumber(item))
end

function RemovePlayerDutyItems(element) -- duty cuccok
	if itemTable[element]["bag"] then
		for i = 1, row * column do
			if (itemTable[element]["bag"][i]) then
				if itemTable[element]["bag"][i]["duty"] == 1 then
					if weaponModels[itemTable[element]["bag"][i]["id"]] then
						delAttachWeapon(element,itemTable[element]["bag"][i]["id"],itemTable[element]["bag"][i]["value"],itemTable[element]["bag"][i]["dbid"])
					end
					deleteItem(element,element,itemTable[element]["bag"][i])
				end
			end
		end
	end
...
addEventHandler("onPlayerQuit",getRootElement(),function()
	if getElementData(source,"user:loggedin") then
		...
		for i = 1, row * column do
			if (itemTable[source]["bag"][i]) then
				if weapons[source] then
					if isElement(weapons[source][itemTable[source]["bag"][i]["dbid"]]) then
						bone:detachElementFromBone(weapons[source][itemTable[source]["bag"][i]["dbid"]])
						destroyElement(weapons[source][itemTable[source]["bag"][i]["dbid"]])
					end
				end
			end
		end
		weapons[source] = {}
        itemTable[source] = nil
	end
end)
...
addEventHandler("moveItemToActionBar", getRootElement(),function(playerSource,itemdbid,item,category,actionslot)
	...
	dbExec(conn,"INSERT INTO actionbaritems SET itemdbid = ?, item = ?, owner = ?, category = ?, actionslot = ?",itemdbid,item,dbid,category,actionslot)
end)

function deleteActionBarItem(player,actionslot)
	local dbid = tonumber(getElementData(player, "user:id"))
	dbExec(conn, "DELETE FROM `actionbaritems` WHERE owner=? and actionslot=?", dbid, actionslot)
end
...
function loadActionBarItems(playerSource)
	...
	end,conn,"SELECT * FROM actionbaritems WHERE owner=?",dbid)
end
