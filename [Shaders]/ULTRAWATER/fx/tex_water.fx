...
PSInput VertexShaderFunctionDB(VSInput VS)
{
    ...
    PS.ProjPos = mul( PS.WorldPos , gViewProjection ); 
    float4 worldView = mul( PS.WorldPos , gView );
    PS.DistFromCam = worldView.z / worldView.w;
...
//------------------------------------------------------------------------------------------
float4 PixelShaderFunctionDB3(PSInput PS) : COLOR0
{
    float projectedX = (0.5 * ( PS.ProjPos.w + PS.ProjPos.x ) );
    float projectedY = (0.5 * ( PS.ProjPos.w - PS.ProjPos.y ) );
    float3 TexCoord = float3( projectedX, projectedY, PS.ProjPos.w );
    float2 TexCoordProj = float2( TexCoord.x, TexCoord.y)/TexCoord.z;  
    TexCoordProj += float2( 0.0006, 0.0006 );
    float BufferValue = FetchDepthBufferValue( TexCoordProj );
    float depth = Linearize( BufferValue );	

    float fade = saturate(((depth - PS.DistFromCam)) * fDepthSpread );
    ...
    float3 h = normalize(vView - gCameraDirection);
    float vdn = saturate(dot(h,vNp2));
    float3 skyColor = gFogColor.rgb * pow(vdn,2) * saturate(PS.WatDistFade);	
    ...
    float4 finalColor = float4(saturate(PS.Diffuse.rgb * vFlakesNormal2.r + envGray * 0.1) * 0.77 , 1);
    finalColor.a = saturate(max(PS.Diffuse.a * 0.25 ,fade ) +  pow(vdn,2) * 0.25) * PS.Diffuse.a;
    finalColor.rgb += skyColor * vFlakesNormal2.r * PS.Diffuse.a * 0.18;
    finalColor.rgb += specLighting * saturate(PS.DistFade) * sVisibility;
    finalColor.a = max( finalColor.a, specLighting * 0.5);
    finalColor.rgb = MTAApplyFog( finalColor.rgb, PS.WorldPos.xyz );
    finalColor.a *= gAlpha;
    return saturate(finalColor);
}

float4 PixelShaderFunctionDB2(PSInput PS) : COLOR0
{
    float projectedX = (0.5 * ( PS.ProjPos.w + PS.ProjPos.x ) );
    float projectedY = (0.5 * ( PS.ProjPos.w - PS.ProjPos.y ) );
    float3 TexCoord = float3( projectedX, projectedY, PS.ProjPos.w );
    ...
    // Bodge in the water color
    float4 finalColor = float4(saturate(PS.Diffuse.rgb + envGray * 0.1) * 0.85 , 1);
    finalColor.a = saturate(max(PS.Diffuse.a * 0.25 ,fade ) +  pow(vdn,2) * 0.25) * PS.Diffuse.a;
    finalColor.rgb += specLighting * saturate(PS.DistFade) * sVisibility;
    finalColor.a = max( finalColor.a, specLighting * 0.5);
    finalColor.a *= gAlpha;
...
// Techniques
//------------------------------------------------------------------------------------------
technique water2DB_sm3
{
    pass P0
    {
        AlphaBlendEnable = TRUE;
        AlphaRef = 1;
        VertexShader = compile vs_3_0 VertexShaderFunctionDB();
        PixelShader  = compile ps_3_0 PixelShaderFunctionDB3();
    }
}

technique water2DB_sm2
{
    pass P0
    {
        AlphaBlendEnable = TRUE;
        ...
        PixelShader  = compile ps_2_0 PixelShaderFunctionDB2();
    }
}

technique water2_noDBuff
{
    pass P0
    {
        AlphaBlendEnable = TRUE;
