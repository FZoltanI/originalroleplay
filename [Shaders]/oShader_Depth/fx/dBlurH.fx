...
float4 PixelShaderFunctionSM3(PSInput PS) : COLOR0
{
    ...
    for(int i = 0; i < 7; ++i)
    {
        float sampleoffset = (origCoC.r * Bright * gblurFactor * Kernel[i])/sTex0Size.x;
        float3 sampleCoC = tex2D(Sampler1, PS.TexCoord.xy + float2(sampleoffset,0)).xyz;
        if(origCoC.y>origCoC.z && sampleCoC.x<origCoC.x) sampleoffset = sampleoffset/origCoC.x*sampleCoC.x;
        coord.x = PS.TexCoord.x + sampleoffset;
