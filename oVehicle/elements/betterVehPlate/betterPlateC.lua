...
local font			= dxCreateFont("elements/betterVehPlate/files/font.ttf", 25, true)
local size			= 1.5
...
for i = 1, 8 do 
    plateBackList[i] = dxCreateShader(raw_shader)
    dxSetShaderValue(plateBackList[i], "platetex", dxCreateTexture("elements/betterVehPlate/files/"..i..".png", "dxt5"))
end

engineApplyShaderToWorldTexture(plateBackList[1], "plateback*")

addEventHandler("onClientElementDataChange", root, function(data, old, new)
    if data == "veh:plateDesign" then 
        engineApplyShaderToWorldTexture(plateBackList[new], "plateback*", source)
    end
end)
